...
def distance_in_cm(pt1, pt2, ppc=pixels_per_cm):
    return euclidean(pt1, pt2) / ppc


def curve_length(points, ppc=pixels_per_cm):
    total = 0
    for i in range(len(points) - 1):
        total += distance_in_cm(points[i], points[i + 1], ppc)
    return total
